...
class Template
{
    ...
    public static function showItemHistory($by, $time)
    {
        $xhtml = sprintf(
            ...
            $by,
            !empty($time) ? date(Config::get('zvn.format.long_time'), strtotime($time)) : ''
        );
        return $xhtml;
    }
    ...
    public static function showItemButton($controllerName, $id, $statusValue, $type)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" class="btn btn-round %s-ajax %s">%s</button>',
            $link,
            $type,
    ...
    public static function showItemIsHome($controllerName, $id, $isHomeValue)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" class="btn btn-round isHome-ajax %s">%s</button>',
            $link,
    ...
    public static function showItemConfigProduct($controllerName, $id, $config, $statusValue)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" data-config="%s" class="btn btn-round config-ajax %s">%s</button>',
            $link,
            $config,
            $currentTemplateStatus['class'],
            $currentTemplateStatus['name']
    ...
    public static function showItemSelect2($controllerName, $id, $arrValue, $displayValue, $fieldName, $showConfig = true)
    {
        ...
        $xhtml = sprintf('<select name="select_change_attr[]" data-url="%s" data-field="%s" class="form-control select-multi select-ajax" multiple>', $link, $fieldName);
    ...
    {
        $xhtml = sprintf(
            '<input type="number" class="form-control input-ordering input-list-admin" name="%s[%s]" data-id="%s" value="%s" min="1"',
            $name,
            $id,
            $id,
            $value
    ...
    public static function uploadImage($name, $value)
    {
        ...
        <div class="form-group d-flex align-items-center">
            <label for="description_short" class="control-label">' . ucwords($name) . '</label>
            <div class="ml-2">
                <div class="d-flex algin-items-center">
                    <div class="input-group mb-0" style="margin-right: 10px">
                        <span class="input-group-btn">
