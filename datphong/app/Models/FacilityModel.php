...
class FacilityModel extends AdminModel implements TranslatableContract
{
    use Translatable;

    protected $table = 'facilities';
    protected $translatedAttributes = ['name', 'head_title', 'head_description', 'main_title', 'main_description'];
    protected $translationForeignKey = 'facility_model_id';
    protected $folderUpload = 'facility';
    protected $searchAccepted = ['name'];
    protected $guarded = ['_token', 'thumb_current', 'thumb_content_current'];
    protected $translationTable = 'facility_translations';
    ...
    public function saveItem($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'add-item') {
            ...
            if (@$params['thumb_content']) {
                $params['thumb_content'] = $this->uploadThumb(@$params['thumb_content']);
            }
        ...
        if ($options['task'] == 'edit-item') {
            ...
            if (!empty($params['thumb_content'])) {
                $this->deleteThumb($params['thumb_content_current']);
                $params['thumb_content'] = $this->uploadThumb($params['thumb_content']);
            }
    ...
    public function deleteItem($params = null, $options = null)
    {
        if ($options['task'] == 'delete-item') {
            ...
            $this->deleteThumb($item->thumb_content);
            $item->delete();
        }
    }

    public function facility_translations()
    {
        return $this->hasMany(FacilityModelTranslation::class, 'facility_model_id', 'id')->locale();
    }
}
