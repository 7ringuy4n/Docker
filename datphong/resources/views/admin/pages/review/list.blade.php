                ...
                @if (count($items) > 0)
                    @foreach ($items as $key => $val)
                        @php
                            ...
                            $country = Highlight::show($val->country, $params['search'], 'country');
                            $message = Highlight::show($val->message, $params['search'], 'message');
                        ...
                        <tr class="{{ $class }} pointer">
                            ...
                            <td>
                                <span>Cleaness</span>
                                {!! Form::text('rating_cleaness', $val->rating_cleaness, ['class' => 'zvn-rating-show']) !!}
                                <span>Staff & Service</span>
                                {!! Form::text('rating_staff_service', $val->rating_staff_service, ['class' => 'zvn-rating-show']) !!}
                            </td>
                            <td>{!! $status !!}</td>
                            <td>{!! $btnOrdering !!}</td>
